...
const requirement = normalizeRequirement(
  getVisaRequirement(props.passportIso2, props.destinationIso2),
)
...
<template>
  <div class="rounded-card border border-gray-200 bg-white p-6">
    ...
    <p class="mt-1 text-sm text-gray-500">Traveling from {{ getCountryName(passportIso2) }}</p>
    ...
    <div v-if="requirement" class="mt-5 rounded-control bg-surface p-4">
      ...
      <p class="mt-1 text-sm text-gray-500">
        Based on passport-index data. Always confirm requirements on the official government site
        before you travel.
      </p>
    </div>
